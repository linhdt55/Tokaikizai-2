...
class StoreBuilder extends Integration {
	...
	protected function getFilters() {
		...
		$filters = [
			...
			[ 'nexcess_mapps_telemetry_report', [ $this, 'collectStoreBuilderSetupData' ] ],
			[ 'nexcess_mapps_telemetry_report', [ $this, 'collectWooCommerceSetupData' ] ],
	...
	public function collectStoreBuilderSetupData( $report = [] ) {
		$report['setup']['storebuilder']['versions'] = [
			'initial' => get_option( self::INITIAL_STOREBUILDER_VERSION, '2.0' ),
	...
	public function maybeUpgradeStoreBuilder() {
		...
		// Bail here if the StoreBuilder setup command was never run.
		...
		$this->v4_migrateSiteBuilderFTCData( $current_storebuilder_version );
		$this->v4_migrateSiteBuilderLookAndFeelData( $current_storebuilder_version );
		$this->v4_migrateSiteBuilderGoLiveData( $current_storebuilder_version );
	 ...
	 *
	 * @param string $current_storebuilder_version
	 */
	protected function v4_migrateSiteBuilderFTCData( $current_storebuilder_version ) {
		if ( version_compare( $current_storebuilder_version, '4.0', '<' ) ) {
			$pre_v3            = version_compare( $current_storebuilder_version, '3.0', '<' );
			$_storebuilder_ftc = get_option( '_storebuilder_ftc' );

			if ( false !== $_storebuilder_ftc ) {
				...
				if ( isset( $_storebuilder_ftc['ftc_complete'] ) ) {
					...
					$_sitebuilder_store_setup['complete'] = ( true === $pre_v3 );
					$_storebuilder_ftc['complete']        = ( true === $pre_v3 );
				}
	 ...
	 * @param string $current_storebuilder_version
	 */
	protected function v4_migrateSiteBuilderLookAndFeelData( $current_storebuilder_version ) {
		if ( version_compare( $current_storebuilder_version, '4.0', '<' ) ) {
			$pre_v3                      = version_compare( $current_storebuilder_version, '3.0', '<' );
			$_storebuilder_look_and_feel = get_option( '_storebuilder_look_and_feel' );

			if ( false !== $_storebuilder_look_and_feel ) {
				$_storebuilder_look_and_feel['complete'] = count( $_storebuilder_look_and_feel ) > 0 || $pre_v3;
	 ...
	 * @param string $current_storebuilder_version
	 */
	protected function v4_migrateSiteBuilderGoLiveData( $current_storebuilder_version ) {
		if ( version_compare( $current_storebuilder_version, '4.0', '<' ) ) {
			$pre_v3                         = version_compare( $current_storebuilder_version, '3.0', '<' );
			...
			$_sitebuilder_go_live = [
				'complete' => $_storebuilder_go_live || $pre_v3,
			];
