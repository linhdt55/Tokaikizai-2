...
<span class="custom-header-wishlist"></span>

<div class="tinv-wishlist woocommerce tinv-wishlist-clear">
	<span class="count_wishlist">
		(<span class="wishlist_products_counter_number"></span>
			<span class="wl-qtys"><?=__('商品','woocommerce');?>)</span>
			<span class="wl-qty"><?=__('商品','woocommerce');?>)</span>
	</span>
	...
	<form action="<?php echo esc_url( $form_url ); ?>" method="post" autocomplete="off"
		...
		<table class="tinvwl-table-manage-list">
			...
			<tr>
				...
				<th class="name-space">&nbsp;</th>

				<?php if ( isset( $wishlist_table_row['colm_price'] ) && $wishlist_table_row['colm_price'] ) { ?>
					<th class="product-price"><?php esc_html_e( '金額', 'ti-woocommerce-wishlist' ); ?></th>
				<?php } ?>
				...
				if ( $wl_product['quantity'] > 0 && apply_filters( 'tinvwl_wishlist_item_visible', true, $wl_product, $product ) ) {
					...
					<tr class="<?php echo esc_attr( apply_filters( 'tinvwl_wishlist_item_class', 'wishlist_item', $wl_product, $product ) ); ?>">
						...
						<td class="product-name">
							...
							// SKUを表示
                            $sku = $product->get_sku(); // SKUを取得
                            if ( $sku ) {
                               echo '<div class="product-sku">' . esc_html__( '商品番号： ', 'ti-woocommerce-wishlist' ) . esc_html( $sku ) . '</div>';
                            }
						...
						<?php if ( isset( $wishlist_table_row['colm_stock'] ) && $wishlist_table_row['colm_stock'] ) { ?>
							<td class="product-stock">
								...
								$availability_html = empty( $availability['availability'] ) ? '<p class="stock ' . esc_attr( $availability['class'] ) . '">
									<span class="wishlist-hidden"><i class="ftinvwl ftinvwl-check"></i></span>
									<span class="tinvwl-txt">' . esc_html__( 'In stock', 'ti-woocommerce-wishlist' ) . '</span></p>' : '<p class="stock ' . esc_attr( $availability['class'] ) . '"><span><i class="ftinvwl ftinvwl-' . ( ( 'out-of-stock' === esc_attr( $availability['class'] ) ? 'times' : 'check' ) ) . '"></i></span><span>' . wp_kses_post( $availability['availability'] ) . '</span></p>';
							...
							<td class="product-action">
								...
								if ( apply_filters( 'tinvwl_wishlist_item_action_add_to_cart', $wishlist_table_row['add_to_cart'], $wl_product, $product ) ) {
									...
									<button class="button alt" name="tinvwl-add-to-cart"
											value="<?php echo esc_attr( $wl_product['ID'] ); ?>"
											title="<?php echo esc_html__( apply_filters( 'tinvwl_wishlist_item_add_to_cart', $wishlist_table_row['text_add_to_cart'], $wl_product, $product ), 'tokaikiza'); ?>">
										<i
