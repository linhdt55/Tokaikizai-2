...
class WC_LINEPay_Admin_Notices {
	...
	public function linepay_check_environment() {
        ...
        $show_end_notice     = get_option( 'wc_linepay_show_end_notice' );
		...
		if ( isset( $options['enabled'] ) && 'yes' === $options['enabled'] ) {
			...
			if ( empty( $show_end_notice ) ){
				$linepay_link = 'https://line-pay-info.landpress.line.me/payment-info/';
				/* translators: 1) LINE Pay End Notice URL */
				$linepay_message = sprintf( __( 'LINE PAY will end its service at the end of May 2025. <a href="%s">Click here for details.</a>', 'woocommerce-for-japan' ), $linepay_link );
				$linepay_message .= __( 'Japanized for WooCommerce will no longer be bundled with the plugin in October 2024. If you wish to use it after that date, please install the standalone plugin.'	, 'woocommerce-for-japan' );
				$this->add_admin_notice( 'linepay_end', 'notice notice-info', $linepay_message , true );
			}
        }
	}
		...
		if ( isset( $_GET['wc-linepay-hide-notice'] ) && isset( $_GET['_wc_linepay_notice_nonce'] ) ) {
			...
			switch ( $notice ) {
				...
				case 'ssl':
					...
					break;
                case 'linepay_end':
                    update_option( 'wc_linepay_show_end_notice', 'no' );
                    break;
			}
		}
	}
